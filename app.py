...
def build_energy_complex_rows(core: Dict[str, Any]) -> List[List[str]]:
    cq = core['current_q']
    nxt = core['next_q']
    rows: List[List[str]] = []
    ...
    if cq == 'Q3':
        add('Oil / gas', 'majors, upstream, LNG, OFS', 'Usable but selective',
            ...
            'If Q2 gets cleaner, shift from hedge logic to cyclical reflation logic.')
        add('Coal', 'thermal coal, miners, coal logistics', 'Usable selectively',
            'Works as dirty-energy / inflation hedge, especially in bad reflation.',
            'If early Q2 confirms, coal becomes more cyclical and less pure hedge.')
        add('Metals', 'copper, steel, aluminum, diversified miners', 'Not confirmed yet',
            ...
            'If Q2 wins cleanly, metals should improve earlier and broader.')
        add('Shipping', 'tankers, LNG shipping, dry bulk, energy transport', 'Selective',
            ...
            'If Q2 wins, leadership can broaden from energy-linked routes to trade-sensitive names.')
        add('Positive spillovers', 'oil services, rail/logistics, industrial suppliers, commodity FX', 'Conditional',
            ...
            'If Q2 wins, these become more volume / cyclical expressions.')
        ...
        add('Oil / gas', 'majors, upstream, LNG, OFS', 'Usable',
            ...
            'If next regime weakens back to Q3/Q4, focus shifts to quality and hedges.')
        add('Coal', 'thermal coal, miners, coal logistics', 'Conditional',
        ...
        add('Metals', 'copper, steel, aluminum, diversified miners', 'More usable',
            ...
            'If Q2 fails, metals tend to lose the clean reflation bid.')
        ...
        add('Pressured losers', 'defensives that lag reflation, long duration', 'Relative laggards',
            ...
            'If Q2 rolls over, these can recover.')
    else:
        ...
        add('Coal', 'thermal coal, miners, coal logistics', 'Mixed',
...
def energy_complex_intro(core: Dict[str, Any]) -> str:
    cq = core['current_q']
    nxt = core['next_q']
...
with t_play:
    ...
    with p1:
        ...
        st.write("")
        st.markdown("**ENERGY / COMMODITY COMPLEX MAP**")
        st.markdown(f"<div class='note-box'>{energy_complex_intro(core)}</div>", unsafe_allow_html=True)
        st.markdown(table_html(["Bucket", "What sits here", "State now", "How to use now", "If next wins"], build_energy_complex_rows(core)), unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
    with p2:
...
- **Energy / commodity complex map** now makes coal, oil/gas, metals, shipping, spillovers, and pressured losers explicit
""")
    st.markdown("</div>", unsafe_allow_html=True)
